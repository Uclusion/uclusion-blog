...
## The decisions outlive the job, not just the session
The same argument scales up a level. A decision is not only invisible to tomorrow's *session* unless something carries it across — it is invisible to your *next job* unless something carries it across. Chat has no answer here: last month's project lives in a different thread the new conversation never opens.

In Uclusion every decision is an addressable object with a stable id that stays reachable after the job that produced it is closed. So an agent — or a teammate — starting a new job does not start cold; it can pull the relevant decisions straight out of a finished, separate job and build on them.

We hit this ourselves: an agent picking up a new job reached into a sibling job that had already wrapped, fetched a resolved decision and the reasoning recorded on it by their ids, and grounded the new work on them — without anyone re-explaining. The short code is the librarian the grep problem was missing. The agent does not scan a transcript and hope it guesses the wording; it asks for the decision by name and gets it back with its status and approval still attached.

Your whole backlog turns into one queryable record of what your team — and your agents — already decided.
